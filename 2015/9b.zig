...
test "example" {
    ...
    try std.testing.expectEqual(@as(u32, 982), result);
}
...
const Graph = struct {
    ...
    pub fn insert(self: *Graph, from: []const u8, to: []const u8, dist: u16) void {
    ...
    pub fn longestRoute(self: Graph) u32 {
        ...
        var longest: u32 = 0;
        self.permuteLongestRoute(&longest, order[0..self.cities.count], 0);
        return longest;
    }

    fn permuteLongestRoute(self: Graph, longest: *u32, order: []u4, i: usize) void {
        if (i == order.len) {
            const total = self.route(order);
            longest.* = @maximum(longest.*, total);
        }

        for (order[i..]) |*j| {
            std.mem.swap(u4, &order[i], j);
            self.permuteLongestRoute(longest, order, i + 1);
            std.mem.swap(u4, &order[i], j);
        }
    }
